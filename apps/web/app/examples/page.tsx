      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {examples.map((example) => (
            <Link key={example.slug} href={example.href} className="group rounded-2xl border border-stone-900/10 bg-white/60 p-6 shadow-sm transition hover:-translate-y-0.5 hover:border-stone-950/25 hover:bg-white">
              <div className="flex items-start justify-between gap-4">
                <h2 className="text-xl font-semibold tracking-tight text-stone-950">{example.title}</h2>
                <span className="font-mono text-xs text-stone-500 group-hover:text-stone-950">{example.slug}</span>
              </div>
              <p className="mt-4 line-clamp-3 text-sm leading-6 text-stone-650 text-stone-700">{example.summary}</p>
            </Link>
          ))}
        </div>
      </div>
